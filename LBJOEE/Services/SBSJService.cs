...
namespace LBJOEE.Services
{
    ...
    public class SBSJService:DBImp<sjcjnew>
    {
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<sjcjnew> Get_NoRunList(string sbbh)
        {
            try
            {
                StringBuilder sql = new StringBuilder();
                ...
                sql.Append(" where  cjsj between sysdate - 30 / (24*60) and sysdate ");
                sql.Append(" and    sbbh = :sbbh ");
                return Db.Connection.Query<sjcjnew>(sql.ToString(), new { sbbh = sbbh });
            }
            catch (Exception e)
            {
                log.Error(e.Message);
