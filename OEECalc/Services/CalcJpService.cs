...
namespace OEECalc.Services
{
    ...
    public class CalcJpService: OracleBaseFixture
    {
        ...
        private static readonly  Lazy<CalcJpService> lazy =  new Lazy<CalcJpService>(() => new CalcJpService());
        private ILog log;
        private CalcJpService()
        {
            log = LogManager.GetLogger(this.GetType());
        }
        public static CalcJpService Instance { get { return lazy.Value; } }
        ...
        public void Update_SBJP_Force(string sbbh,int beforehours = 5)
        {
            try
            {
                    ...
                    var list = db.Query<sys_cjsj>(sql.ToString(), p).ToList();
                    ...
                    for (int i = 0; i + 1 < list.Count; i++)
                    {
                        ...
                        var res = db.Execute(sql1.ToString(), dp);
                        if (res > 0)
                        {
                        ...
                        if (i == list.Count - 2)
                        {
                            ...
                            var q = db.Query<sys_cjsj>(tsql.ToString(), dyp);
                            if (q.Count() > 0)
                            {
                                ...
                                dp.Add(":rid", rowid, System.Data.DbType.String, System.Data.ParameterDirection.Input);
                                dp.Add(":jp", jp, System.Data.DbType.String, System.Data.ParameterDirection.Input);
                                res = db.Execute(sql1.ToString(), dp);
                                if (res > 0)
                                {
                                    Db.Insert<sbjpgxb>(new sbjpgxb()
                                    {
